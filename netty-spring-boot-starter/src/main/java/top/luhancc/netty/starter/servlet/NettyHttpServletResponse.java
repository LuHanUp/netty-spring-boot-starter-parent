...
public class NettyHttpServletResponse implements HttpServletResponse {
    ...
    private String getCookieHeader(Cookie cookie) {
        ...
        if (maxAge >= 0) {
            ...
            buf.append("; Expires=");
            ZonedDateTime expires = (cookie instanceof NettyCookie ? ((NettyCookie) cookie).getExpires() : null);
            if (expires != null) {
        ...
        if (cookie instanceof NettyCookie) {
            ...
            if (StringUtils.hasText(NettyCookie.getSameSite())) {
                buf.append("; SameSite=").append(NettyCookie.getSameSite());
            }
        }
        return buf.toString();
    }
    ...
    private boolean setSpecialHeader(String name, Object value, boolean replaceHeader) {
        if (HttpHeaders.CONTENT_TYPE.equalsIgnoreCase(name)) {
            ...
            NettyCookie cookie = NettyCookie.parse(value.toString());
            if (replaceHeader) {
                setCookie(cookie);
            } else {
                addCookie(cookie);
