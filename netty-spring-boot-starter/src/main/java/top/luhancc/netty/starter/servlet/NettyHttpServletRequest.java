...
//                ((NettyServletContext) this.servletContext).getDeclaredRoles().contains(role)));
    ...
    public void setSession(HttpSession session) {
        ...
        if (session instanceof NettyHttpSession) {
            NettyHttpSession mockSession = ((NettyHttpSession) session);
            mockSession.access();
        }
    }
     ...
     * {@link NettyHttpSession#changeSessionId()} if the session is a mock session.
    ...
    public String changeSessionId() {
        ...
        if (this.session instanceof NettyHttpSession) {
            return ((NettyHttpSession) this.session).changeSessionId();
        }
